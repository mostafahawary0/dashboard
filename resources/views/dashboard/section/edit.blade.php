...
if(field3 == "")
{
document.getElementById('xfield3').innerHTML = " <b style='font-size:17px; font-weight:bold;  color:#f00;'>{{__('lang.ValidSecAR')}}  </b>";
...
<li class="active" style="padding-left:10px; padding-right:10px;" aria-current="page">
{{__('lang.edit')}}
</li>
</ol>
</nav>
...
<form action="{{route('section.update',$sections->id)}}" method="POST"  onsubmit="return save()">
@method('PUT')
@csrf
<div class="card-body" >
...
class="form-control @error('nameAR') is-invalid @enderror" value="{{$sections->nameAR}}">
@error('nameAR')
...
class="form-control @error('nameEN') is-invalid @enderror"  value="{{$sections->nameEN}}">
@error('nameEN')
<div class="alert alert-danger">{{ $message }}</div>
@enderror   
<div id="xfield2"></div>
...
class="form-control @error('nameFR') is-invalid @enderror" value="{{$sections->nameFR}}">
@error('nameFR')
<div class="alert alert-danger">{{ $message }}</div>
@enderror   
<div id="xfield3"></div>
